...
export class EmulatorManager {
    ...
    async startGame(romName, Module) {
        ...
        console.log("MainLoopTimingMode: " + Module.getMainLoopTimingMode());
        console.log("MainLoopTimingValue: " + Module.getMainLoopTimingValue());
    }
    ...
    async initializeEmulator() {
        ...
        const mGBAModule = await mGBA(Module);
        this.Module = mGBAModule;

        // Expose Module to window for virtual controls
        window.Module = mGBAModule;

        let mGBAVersion = mGBAModule.version.projectName + ' ' + mGBAModule.version.projectVersion;
        console.log(mGBAVersion);

        // Add delay to see mGBA loading state
        await this.delay(1500);

        this.loadingManager.updateText('Initializing filesystem...');

        // Add delay to see filesystem initialization state
        await this.delay(1000);

        await mGBAModule.FSInit();

        this.loadingManager.updateText('Loading ROM...');

        // Add delay to see ROM loading state
        await this.delay(800);

        const romData = await this.getROMFile(this.romName, this.romCacheKey);
        await this.setupEmulator(romData, this.romName, mGBAModule);
        await this.startGame(this.romName, mGBAModule);
    }
}
